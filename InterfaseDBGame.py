...
class WorkWithDB:
    ...
    def sign_up_in_game(self, login:str, password:str):
        record = self.req_select_db("name_user", "users",
                "name_user = '{}'".format(login))
        if not record:
    ...
    def updata_score(self, login:str, score_user:int, number_level:int):
        id_user = self.req_select_db("id","users",
                "name_user='{}'".format(login))      
        current_score = self.req_select_db("score","rating",
                "id_user = {} and number_level = {}".format(
                id_user[0][0], number_level))
        if int(current_score[0][0]) < int(number_level):
            values = "nextval('rating_id_seq'),{},now(),'{}',{})".format(
                    score_user, id_user, id_user)
            self.req_insert_db("rating",values,"ignore")
